...
def get_tool_description(tool_name: str) -> str:
    ...
    for tool_name , tool in tools.items():
        original_function = getattr(tool, "__wrapped__", None)
...
def run_agent(question: str):
    ...
    print("=" * 60)
    react_prompt_with_question = react_prompt.format(question=question)
    scratchpad = ""

    for iteration in range(1, MAX_ITERATIONS + 1):
        print(f"\n--- Iteration {iteration} ---")
        full_prompt = f"{react_prompt_with_question}\n\n{scratchpad}"
